...
def interactive_main():
	print('-'*100)
	run_name = input('Enter a name for storing your results (Results are stored in ./name/)\nName can contain \
numbers,letters,underscore only\n:')
	print('-'*100)
	results_path = './{}'.format(run_name)
	os.system('mkdir ./{}'.format(run_name))
	os.system('mkdir ./{}/temp'.format(run_name))
	print('-'*100)
...
def main(args):
	...
	results_path = os.path.abspath('./{}'.format(run_name))
	...
	try:
		os.mkdir(os.path.abspath('./{}/temp'.format(run_name)))
	except:
		pass
...
if __name__ == '__main__':
	...
	parser.add_argument("run_name", help="'Enter a {name} for storing your results, {name} cannot have spaces (Results are stored in directory ./{name})", type=str)
